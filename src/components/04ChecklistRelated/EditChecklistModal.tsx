...
export default function EditChecklistModal({ prototypeId, checklist, onClose, onSave }: Props) {
    // estado local inicializado com cópia profunda do prop checklist
    const [localChecklist, setLocalChecklist] = useState<Checklist>(() => JSON.parse(JSON.stringify(checklist)));
    ...
    const handleSave = async () => {
        ...
        setSaving(true);
        try {
            // toggleChecklistItems retorna true/ null conforme seu serviço
            const res = await toggleChecklistItems(prototypeId, localChecklist.id, localChecklist);
            if (res) {
                // como seu serviço retorna boolean, usamos a cópia local para atualizar a UI
                onSave(JSON.parse(JSON.stringify(localChecklist)));
            } else {
                console.error("toggleChecklistItems retornou falsy.");
                alert("Não foi possível salvar a checklist. Tente novamente.");
            }
        } catch (err) {
